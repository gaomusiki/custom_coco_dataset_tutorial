...
def convert_int64_to_int(d):
    if isinstance(d, dict):
        for key, value in d.items():
            d[key] = convert_int64_to_int(value)  # 递归转换字典中的值
    elif isinstance(d, list):
        d = [convert_int64_to_int(item) for item in d]  # 递归转换列表中的元素
    elif isinstance(d, np.int64):
        return int(d)  # 将 numpy.int64 转换为 int
    return d
...
def convert_detection_to_panoptic_coco_format_single_core(
    ...
    annotations_panoptic = convert_int64_to_int(annotations_panoptic)
...
def convert_detection_to_panoptic_coco_format(input_json_file,
    ...
    if segmentations_folder is None:
        segmentations_folder = "./panoptic_train"
